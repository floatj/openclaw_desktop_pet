...
2. **基本 SSE 不能推遲**。如果要讓 Mini App 裡的兔兔跟 OpenClaw 回覆同步，Layer 2 最少需要 SSE。推遲的是多人 presence、共同操作、WebSocket 遊戲式同步。
3. **「兔兔正在...」卡片分兩階段**。MVP 先在 Mini App 顯示狀態；Telegram 群組裡的可編輯狀態卡片等到 OpenClaw tool/progress events 穩定後再做。
...
> 兔兔不是 3D 模型。兔兔是一個活在 Telegram 裡的小生命，3D 是她的身體，OpenClaw 是她的大腦，Telegram 是她生活的地方。
...
- 不是 OpenClaw Gateway 的公開前端。
- 不是需要大家每天維護的養成系統。

### 是

- 一個能被分享到 Telegram 群組的 3D 兔兔房間。
- 一個會根據 OpenClaw 與群組互動改變狀態的小生命。
- 一個低打擾的群組陪伴存在。
...
Layer 2: OpenClaw hook + SSE 事件連動
  目標：OpenClaw 收到/回覆訊息時，兔兔同步反應。
...
| 書桌 | 思考/工作 | OpenClaw think/speak 時靠近 |
...
OpenClaw 的 Telegram 收訊和回覆會驅動兔兔動作。

### 架構

```text
OpenClaw Telegram runtime
...
| OpenClaw event | Rabbit event | 狀態變化 |
|---|---|---|
...
- 真正的進度百分比需要 OpenClaw 更細的 lifecycle/tool events。
...
### Phase 2: OpenClaw-Reactive Rabbit

目標：OpenClaw 事件能驅動兔兔。

Deliverables:

- `rabbit-avatar` OpenClaw hook
- local SSE relay
...
- OpenClaw 回覆 Telegram 時，Mini App 裡兔兔會同步 `speak`。
...
- camera capture
- image upload to OpenClaw/backend
- fake AR share image
...
5. `/兔兔`, `/摸摸兔兔`, `/兔兔心情`。
6. SSE relay。
7. OpenClaw `message:received` / `message:sent` hook。
...
3. OpenClaw hook 能拿到多少 Telegram group metadata。
